 ...
 **                  初始化  ：  vW25qx_Init()　
 **                  读取数据：  vW25qxx_ReadBuffer( uint32_t addr, uint8_t *p, uint16_t num)　 // 数据地址，缓存指针，字节数
 **                  写入数据：  vW25qxx_WriteBuffer(uint32_t addr, uint8_t *p, uint16_t num)   // 数据地址，缓存指针，字节数
 **
...
void W25qxx_ReadBuffer(uint32_t addr, uint8_t *buf, uint16_t num);       // 读数据：addr-地址，*p-读取后数据缓存，num-要读取的字节数
void W25qxx_WriteBuffer(uint32_t addr, uint8_t *buf, uint16_t num);      // 写数据：addr-地址，*p-待写的数据缓存，num-要写入的字节数
void W25qxx_ReadGBK(uint8_t *typeface, uint8_t size, uint8_t *dataBuf);  // 从w25qxx的字库中读取出字模数据
