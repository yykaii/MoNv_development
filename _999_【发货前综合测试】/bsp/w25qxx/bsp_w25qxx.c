...
#if 1
static void delay_us( __IO u32 times)         // 定义一个us延时函数，减少移植时对外部文件依赖; 本函数仅作粗略延时使用，而并非精准延时;
{
...
void W25qx_Init()
{
    ...
    if(W25QXX_NSS_GPIO == GPIOG)  RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOG , ENABLE);    
    // 使能SPIx引脚端口时钟
