...
#include "stm32f10x_tim.h"
    TIM_TimeBaseInitTypeDef  TIM_TimeBaseStructure;
    RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM7, ENABLE);      // 开启TIM6时钟,即内部时钟CK_INT=72M
    TIM_Cmd(TIM7, DISABLE);                                   // 在配置前，先关闭计数器
    TIM_TimeBaseStructure.TIM_Prescaler = PSC - 1;            // 预分频器数值,16位; 计数器的时钟频率CK_CNT等于f CK_PSC /(PSC[15:0]+1)。在每一次更新事件时，PSC的数值被传送到实际的预分频寄存器中。
    TIM_TimeBaseStructure.TIM_Period    = ARR - 1;            // 自动重装载数值,16位; 即多少个脉冲产生一个更新或中断(1周期)。如果自动重装载数值为0，则计数器停止。
    TIM_TimeBaseInit(TIM7, &TIM_TimeBaseStructure);           // 初始化定时器

    TIM_ClearFlag(TIM7, TIM_FLAG_Update);                     // 清除计数器中断标志位
    TIM_ITConfig(TIM7, TIM_IT_Update, ENABLE);                // 开启更新中断(只是开启功能，未工作）
#else                                                         // 寄存器写法, 优点：方便移植、调试
    RCC->APB1ENR |= RCC_APB1ENR_TIM7EN ;                      // 开启TIM6时钟,
    TIM7->CR1  = 0;                                           // 在配置前，先关闭计数器, 并清0 CR1的配置
    TIM7->CR2  = 0;                                           // 清0 CR2的配置
    TIM7->SR   = 0;                                           // 清0 SR寄存器, 本寄存器只有一个功能: 中断标志位。
    TIM7->CNT  = 0;                                           // 清0 计数器的值
    TIM7->DIER = 0x01 ;                                       // 开启更新中断(只是开启功能，未工作）, 关闭DMA清求
    TIM7->CR1 |= 1 << 7;                                      // 开启预装载
    TIM7->PSC  = PSC - 1;                                     // 预分频器数值,16位; 计数器的时钟频率CK_CNT等于f CK_PSC /(PSC[15:0]+1)。在每一次更新事件时，PSC的数值被传送到实际的预分频寄存器中。
    TIM7->ARR  = ARR - 1;                                     // 自动重装载数值,16位; 即多少个脉冲产生一个更新或中断(1周期)。如果自动重装载数值为0，则计数器停止。
#endif
}
...
{
    TIM7->SR &= (uint16_t)~0x01;    // 清0更新中断标志位
    xTIM.TIM7Counter ++ ;           // 计数+1
}
