 ...
 ** 本地变量声明
 *****************************************************************************/
uint64_t sysTickCnt = 0;                    // 运行时长，单位：ms
_flag xFlag;                           // 全局状态标志
...
{       
    ...
    uint32_t msTick= SystemCoreClock /1000;     // 计算重载值，全局变量SystemCoreClock的值 ， 定义在system_stm32f10x.c    
 ...
 * 返回值： uint32_t us
*****************************************************************************/
uint32_t System_GetTimeUs(void)
{
    uint32_t ms;
    uint32_t us;
    do{
...
{
    uint64_t nowUs = System_GetTimeUs ();
 ...
 *****************************************************************************/ 
uint32_t System_GetTimeInterval(void)
{
    static uint32_t  lastTime=0, nowTime=0;
...
void System_TestRunTimes(void)
{
    ...
    uint32_t intervalTimes = System_GetTimeInterval ();
    if(intervalTimes == 0)
    {
 ...
 * 返回值： 
******************************************************************************/
void System_GPIOSet(GPIO_TypeDef* GPIOx, uint32_t allPin, uint8_t mode, uint8_t speed)
{
    uint32_t reg = 0; 
    uint32_t nowPin=0;    
...
//    }      
    ...
    {      
        nowPin = (uint32_t) 0x01 << i;         // 当前要判断的引脚号     
    ...
    for(uint32_t i=0; i<8; i++)    
    {      
        nowPin = (uint32_t) 0x01 << (i+8);     // 当前要判断的引脚号     
        if((allPin & nowPin) != 0)        // 当前引脚要配置
        {
 ...
 * 返回值： 
******************************************************************************/
void System_NVICSet(uint8_t NVIC_Channel, uint8_t Preemption)
{    
    static uint8_t setGrouped=0;
    if(setGrouped ==0){
        // 全局分级设置,统一为组4, 值0b11,即：NVIC->IPx中高4位:主级4位(16级), 子级0位(0级）  
        SCB->AIRCR = ((uint32_t)0x05FA0000)|(0x03<<8);   // 优先级分组设置, 已查,是3， F103和F429寄存器通用        
        setGrouped =1;
    }
 ...
 * 返  回： 
*****************************************************************************/
void System_EXTISet(GPIO_TypeDef* GPIOx, uint16_t PINx, uint8_t TRIM)
{
    uint8_t gpioNum = 0;
    uint8_t pinNum  = 0;
    ...
    for(uint8_t i=0; i<16; i++){
        if( PINx== ((uint32_t)1<<i)){
            pinNum=i;
            break;
        }          
    }    
    
    uint8_t offSet   = (pinNum%4)*4;                    // 寄存器内偏移
 ...
 **
**  说  明:  1_把数据存储于STM32芯片内部, 只适合少量数据，如1K; 不适合几十K的数据;
