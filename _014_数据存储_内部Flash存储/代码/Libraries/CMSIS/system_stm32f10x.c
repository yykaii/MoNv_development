...
{
    // 复位时钟至默认状态
    RCC->CR   |= (uint32_t)0x00000001;                                // 开启内部时钟
    RCC->CFGR &= (uint32_t)0xF8FF0000;                                // 复位 SW, HPRE, PPRE1, PPRE2, ADCPRE and MCO bits
    RCC->CFGR &= (uint32_t)0xFF80FFFF;                                // 复位 PLL和分频的配置
    ...
    if ((RCC->CR & RCC_CR_HSERDY) != RESET)  {  
        ...
        RCC->CFGR  |= (uint32_t)RCC_CFGR_HPRE_DIV1;                   // [7:4]   AHB  预分频, HCLK = SYSCLK/1  不分频     
        RCC->CFGR  |= (uint32_t)RCC_CFGR_PPRE2_DIV1;                  // [13:11] APB2 预分频, APB2 = HCLK/1,   不分频
        RCC->CFGR  |= (uint32_t)RCC_CFGR_PPRE1_DIV2;                  // [10: 8] APB1 预分频, APB1 = HCLK/2,    2分频

        RCC->CFGR  &= (uint32_t)(~( 1<<16 | 0x01<<17 | 0xF<<18));     // 清零
        RCC->CFGR  |= (uint32_t)(0x01<<16 | 0x07<<18);                // PLL 时钟源,低频因子，倍频系数， 使PLLCK= HSE * 9= 72MHz   
        RCC->CR    |= (uint32_t)(0x01<<24);                           // 使能 PLL    
        while((RCC->CR & RCC_CR_PLLRDY) == 0) {  }               // 等待PLL就绪
    
        RCC->CFGR &= (uint32_t)((uint32_t)~(0x3<<0));                      // 清0
        RCC->CFGR |= (uint32_t)(0x1 << 1);                            // 切换系统时钟源为：PLLCLOCK    
