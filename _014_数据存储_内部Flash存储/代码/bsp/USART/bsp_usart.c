...
void USART1_IRQHandler(void)
{
    ...
    if (USART1->SR & (1 << 4))                                       // 检查IDLE(空闲中断标志位); IDLE中断标志清理方法：序列清零，USART1 ->SR;  USART1 ->DR;
    {
        ...
        xUSART.USART1ReceivedFlag = 1;                               // 标记；外部程序通过检查xUSARTFlag.USARTxReceived是否等于1, 可判断是否有新一帧数据
        xUSART.USART1ReceivedCNT  = cnt;                             // 把接收到的字节数，存放到全局变量xUSART.USARTxReceivedCNT中；
...
{
    if (xUSART.USART1ReceivedFlag == 1)     // 判断是否有新数据
    {
        memcpy(buffer, xUSART.USART1ReceivedBuffer, xUSART.USART1ReceivedCNT);   // 把新数据复制到指定位置
        *cnt = xUSART.USART1ReceivedCNT;    // 把新数据的字节数，存放指定变量
        xUSART.USART1ReceivedFlag = 0;      // 接收标记置0
        return xUSART.USART1ReceivedCNT;    // 返回所接收到新数据的字节数
    }
...
void USART1_SendString(char *stringTemp)
{
    ...
    USART1_SendData((uint8_t *)stringTemp, num + 1);  // 调用函数完成发送，num+1：字符串以0结尾，需多发一个:0
}
...
void USART2_IRQHandler(void)
{
    ...
    if (USART2->SR & (1 << 4))                                       // 检查IDLE(空闲中断标志位); IDLE中断标志清理方法：序列清零，USART1 ->SR;  USART1 ->DR;
    {
        ...
        xUSART.USART2ReceivedFlag = 1;                               // 标记；外部程序通过检查xUSARTFlag.USARTxReceived是否等于1, 可判断是否有新一帧数据
        xUSART.USART2ReceivedCNT  = cnt;                             // 把接收到的字节数，存放到全局变量xUSART.USARTxReceivedCNT中；
...
{
    if (xUSART.USART2ReceivedFlag == 1)     // 判断是否有新数据
    {
        memcpy(buffer, xUSART.USART2ReceivedBuffer, xUSART.USART2ReceivedCNT);   // 把新数据复制到指定位置
        *cnt = xUSART.USART2ReceivedCNT;    // 把新数据的字节数，存放指定变量
        xUSART.USART2ReceivedFlag = 0;      // 接收标记置0
        return xUSART.USART2ReceivedCNT;    // 返回所接收到新数据的字节数
    }
    return 0;                               // 返回0, 表示没有接收到新数据
}
...
void USART2_SendString(char *stringTemp)
{
    ...
    char *t = stringTemp ;                       // 用于配合计算发送的数量
    while (*t++ != 0)  num++;                    // 计算要发送的数目，这步比较耗时，测试发现每多6个字节，增加1us，单位：8位
    USART2_SendData((uint8_t *)stringTemp, num + 1);  // 调用函数完成发送，num+1：字符串以0结尾，需多发一个:0
}
...
void USART3_IRQHandler(void)
{
    ...
    if (USART3->SR & (1 << 4))                                       // 检查IDLE(空闲中断标志位); IDLE中断标志清理方法：序列清零，USART1 ->SR;  USART1 ->DR;
    {
        ...
        xUSART.USART3ReceivedFlag = 1;                               // 标记；外部程序通过检查xUSARTFlag.USARTxReceived是否等于1, 可判断是否有新一帧数据
        xUSART.USART3ReceivedCNT  = cnt;                             // 把接收到的字节数，存放到全局变量xUSART.USARTxReceivedCNT中；
...
{
    if (xUSART.USART3ReceivedFlag == 1)     // 判断是否有新数据
    {
        memcpy(buffer, xUSART.USART3ReceivedBuffer, xUSART.USART3ReceivedCNT);   // 把新数据复制到指定位置
        *cnt = xUSART.USART3ReceivedCNT;    // 把新数据的字节数，存放指定变量
        xUSART.USART3ReceivedFlag = 0;      // 接收标记置0
        return xUSART.USART3ReceivedCNT;    // 返回所接收到新数据的字节数
    }
    return 0;                               // 返回0, 表示没有接收到新数据
}
...
void USART3_SendString(char *stringTemp)
{
    ...
    char *t = stringTemp ;                       // 用于配合计算发送的数量
    while (*t++ != 0)  num++;                    // 计算要发送的数目，这步比较耗时，测试发现每多6个字节，增加1us，单位：8位
    USART3_SendData((uint8_t *)stringTemp, num + 1);  // 调用函数完成发送，num+1：字符串以0结尾，需多发一个:0
}
...
void UART4_IRQHandler(void)
{
    ...
    if (UART4->SR & (1 << 4))                                       // 检查IDLE(空闲中断标志位); IDLE中断标志清理方法：序列清零，USART1 ->SR;  USART1 ->DR;
    {
        ...
        xUSART.UART4ReceivedFlag = 1;                               // 标记；外部程序通过检查xUSARTFlag.USARTxReceived是否等于1, 可判断是否有新一帧数据
        xUSART.UART4ReceivedCNT  = cnt;                             // 把接收到的字节数，存放到全局变量xUSART.USARTxReceivedCNT中；
...
{
    if (xUSART.UART4ReceivedFlag == 1)     // 判断是否有新数据
    {
        memcpy(buffer, xUSART.UART4ReceivedBuffer, xUSART.UART4ReceivedCNT);   // 把新数据复制到指定位置
        *cnt = xUSART.UART4ReceivedCNT;    // 把新数据的字节数，存放指定变量
        xUSART.UART4ReceivedFlag = 0;      // 接收标记置0
        return xUSART.UART4ReceivedCNT;    // 返回所接收到新数据的字节数
    }
    return 0;                              // 返回0, 表示没有接收到新数据
}
...
void UART4_SendString(char *stringTemp)
{
    ...
    char *t = stringTemp ;                       // 用于配合计算发送的数量
    while (*t++ != 0)  num++;                    // 计算要发送的数目，这步比较耗时，测试发现每多6个字节，增加1us，单位：8位
    UART4_SendData((uint8_t *)stringTemp, num + 1);   // 调用函数完成发送，num+1：字符串以0结尾，需多发一个:0
}
...
void UART5_IRQHandler(void)
{
    ...
    if (UART5->SR & (1 << 4))                                       // 检查IDLE(空闲中断标志位); IDLE中断标志清理方法：序列清零，USART1 ->SR;  USART1 ->DR;
    {
        ...
        xUSART.UART5ReceivedFlag = 1;                               // 标记；外部程序通过检查xUSARTFlag.USARTxReceived是否等于1, 可判断是否有新一帧数据
        xUSART.UART5ReceivedCNT  = cnt;                             // 把接收到的字节数，存放到全局变量xUSART.USARTxReceivedCNT中；
...
{
    if (xUSART.UART5ReceivedFlag == 1)     // 判断是否有新数据
    {
        memcpy(buffer, xUSART.UART5ReceivedBuffer, xUSART.UART5ReceivedCNT);   // 把新数据复制到指定位置
        *cnt = xUSART.UART5ReceivedCNT;    // 把新数据的字节数，存放指定变量
        xUSART.UART5ReceivedFlag = 0;      // 接收标记置0
        return xUSART.UART5ReceivedCNT;    // 返回所接收到新数据的字节数
    }
    return 0;                              // 返回0, 表示没有接收到新数据
}
...
void UART5_SendString(char *stringTemp)
{
    ...
    char *t = stringTemp ;                       // 用于配合计算发送的数量
    while (*t++ != 0)  num++;                    // 计算要发送的数目，这步比较耗时，测试发现每多6个字节，增加1us，单位：8位
    UART5_SendData((uint8_t *)stringTemp, num + 1);   // 调用函数完成发送，num+1：字符串以0结尾，需多发一个:0
}
...
int fputc(int ch, FILE *f)                   // 重定向fputc函数，使printf的输出，由fputc输出到UART,  这里使用串口1(USART1)
{
    ...
    USARTx_DEBUG->DR = (uint8_t) ch;          // 写DR,串口1将发送数据
    return ch;
