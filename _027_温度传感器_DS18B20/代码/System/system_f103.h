 ...
 **【 功  能 】  位操作宏定义，常用功能函数声明
 **
 **【主要函数】  1-SysTick定时器应用部分
 **                System_SysTickInit (void);                                                         // 配置SysTick时钟，配置后System_DelayMS()、System_DelayUS()即可使用
 **                System_DelayMS (uint32_t);                                                         // 毫秒延时
 **                System_DelayUS (uint32_t);                                                         // 微秒延时
 **                System_GetTimeMs (void);                                                           // 获取 SysTick 计时数, 单位:ms
 **                System_GetTimeInterval (void);                                                     // 监察运行时间
 **                System_TestRunTimes (void);                                                        // printf打印监察运行时间
 **
 **              2-简化初始化代码的3大函数
 **                System_GPIOSet(GPIO_TypeDef* GPIOx, uint32_t allPin, uint8_t mode, uint8_t speed); // GPIO初始化
 **                System_NVICSet (uint8_t NVIC_Channel, uint8_t Preemption);                         // NVIC优先级配置, 已分好组, 4位抢占级, 16级, 无子级
 **                System_EXTISet (GPIO_TypeDef* GPIOx, uint16_t PINx, uint8_t TRIM);                 // 外部中断配置函数
 **
 **              3-辅助功能
 **                System_SwdMode (void);                                                             // SWD调试模式，关闭JTAG只保留SWD，可释放引脚PB3、PB4、PA15，只需PA13、PA14
 **                System_GetSystemClock (void);                                                      // 获取系统时钟频率
 **                System_Reset (void);                                                               // 系统软复位
 **                System_MCO1Init (uint32_t);                                                        // 可选参数,前辍 RCC_MCO1Source_ +++ HSI , LSE , HSE , PLLCLK
 **                System_MCO2Init (uint32_t);                                                        // 可选参数,前辍 RCC_MCO2Source_ +++ SYSCLK , PLLI2SCLK , HSE , PLLCLK
 **                System_ReadInteriorFlash (uint32_t addr, uint8_t *buf, uint16_t num);              // 芯片内部FLASH存储数据函数: 读取指定长度数据
 **                System_WriteInteriorFlash(uint32_t addr, uint8_t *buf, uint16_t num);              // 芯片内部FLASH存储数据函数: 写入指定长度数据
 **
 **
 **【移植使用】  复制工程文件夹下的system_f103.c和h两个文件，粘贴到目标文件夹即可。
 **              
 **
 **【更新记录】  2021-09-07  增加内部FLASH数据存取函数
 **              2020-03-20  完善注释
 **
***********************************************************************************************************************************/
...
typedef struct
{
    ...
    uint8_t  usartRecived;              // 调试所用的串口接收到数据后，等待处理标志， uint8_t RxBuffer[255]
...
//
typedef  volatile   uint8_t   vuint8_t;   // volatile 每一次读取都要从内存读取，防止被错误地优化了
...
void      System_DelayMS(uint32_t);       // 毫秒延时
void      System_DelayUS(uint32_t);       // 微秒延时
uint64_t  System_GetTimeMs(void);         // 获取 SysTick 计时数, 单位:ms
uint32_t  System_GetTimeInterval(void);   // 监察运行时间
void      System_TestRunTimes(void);      // printf打印监察运行时间
// 简化初始化代码的3大函数
void      System_GPIOSet(GPIO_TypeDef *GPIOx, uint32_t allPin, uint8_t mode, uint8_t speed);  // GPIO初始化
void      System_NVICSet(uint8_t NVIC_Channel, uint8_t Preemption);                           // NVIC优先级配置, 已分好组, 4位抢占级, 16级, 无子级
void      System_EXTISet(GPIO_TypeDef *GPIOx, uint16_t PINx, uint8_t TRIM);                   // 外部中断配置函数
// 辅助功能
void      System_SwdMode(void);                                                   // SWD调试模式，关闭JTAG只保留SWD，可释放引脚PB3、PB4、PA15，只需PA13、PA14
uint32_t  System_GetSystemClock(void);                                            // 获取系统时钟频率
...
uint8_t   System_ReadInteriorFlash(uint32_t addr, uint8_t *buf, uint16_t num);    // 芯片内部FLASH存储数据函数: 读取指定长度数据
uint8_t   System_WriteInteriorFlash(uint32_t addr, uint8_t *buf, uint16_t num);   // 芯片内部FLASH存储数据函数: 写入指定长度数据
#endif
