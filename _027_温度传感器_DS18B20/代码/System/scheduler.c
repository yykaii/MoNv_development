...
void vTask_100ms(void)
{
    ...
    static uint16_t CNT_RS485 = 0;
    //static uint16_t CNT_ESP8266=0;

    if (xUSART.USART1ReceivedFlag == 1)
    {
        // 1：打印到串口上位机
        sprintf(strTemp, "USART1接收到新一帧数据：%s\r\n", (char *)xUSART.USART1ReceivedBuffer);
        ...
        USART1_SendString(strTemp);
        // 3: 通过RS485总线发出
        USART3_SendString((char *)xUSART.USART1ReceivedBuffer);
        sprintf(strTemp, "\r\nRS485发送的数据:>>> %s\r\n", (char *)xUSART.USART1ReceivedBuffer);
        USART1_SendString(strTemp);
        // 4: 通过ESP8266发出
        ...
        // 清理USART1的接收标志
        // 5: 重新校准
        if (strstr((char *)xUSART.USART1ReceivedBuffer, "XPT2046") || strstr((char *)xUSART.USART1ReceivedBuffer, "xpt2046"))
        {
            XPT2046_ReCalibration();
        }
        xUSART.USART1ReceivedFlag = 0;
    }
    ...
    // 3：RS485接收处理
    if (xUSART.USART3ReceivedFlag == 1)                          // 检查RS485所用的UART4是否收到数据
    {
        sprintf(strTemp, "\rRS485接收到第%3d次数据：%s\r", ++CNT_RS485, (char *)xUSART.USART3ReceivedBuffer);   // 格式化字符串
        USART1_SendString(strTemp);                              // 通过UART1把数据输出到电脑上位机
        xUSART.USART3ReceivedFlag = 0;                           // 处理完后，把RS485所用的UART4接收到数据标志清0，以方便下一帧数据的接收
        // 判断是否测试工具发过来的数据
        if (strcmp((char *)xUSART.USART3ReceivedBuffer, "RS485_Test") == 0)
        {
            USART3_SendString("RS485_OK");
        }
    }// end if RS485
