 ...
 *              void TIM6_Init(uint16_t PSC, uint16_t ARR);    // 时钟分频值、自动重装载值
 *              void TIM6_Start();                             // 启动TIM6, 以TIM6_Init函数的配置开始工作
 *              void TIM6_Stop();                              // 停止工作，
 *
...
int main(void)
{
    ...
    TIM6_Init(72, 1000);                             // TIM6初始化，分频值:72, 重装载值：1000，即：每个脉冲0.0000001秒(1MHz: 72000000/7200), 每1000次脉冲产生一次中断(1000*0.000001=1ms);
    TIM6_Start();
    
    while (1)                                        // while函数死循环，不能让main函数运行结束，否则会产生硬件错误
    {
        if (xTIM.TIM6Counter >= 500)                 // 每500ms执行一次
        {
            LED_RED_TOGGLE;                          // 红色LED 每0.5秒闪灭一次，以监察系统正常工作
            LED_BLUE_TOGGLE;
            xTIM.TIM6Counter = 0;                    // 计数清0，以执行下一次计数
        }
    }
}
