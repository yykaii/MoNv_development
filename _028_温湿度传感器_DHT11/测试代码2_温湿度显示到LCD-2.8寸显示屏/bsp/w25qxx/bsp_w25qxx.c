...
static void  eraseSector(uintt32_t addr);                   // 5_4    擦扇区
static void  writeSector(uintt32_t addr, u8* p, u16 num);   // 5_5    写扇区
// 测试
...
#if 1
static void delayUS( __IO uintt32_t times)
{
...
void W25qx_Init()
{
    ...
    if(W25QXX_CLK_GPIO == GPIOG)  RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOF , ENABLE);
...
void eraseSector(uintt32_t addr)
{
...
// 5_5 写扇区. 要分页写入
void writeSector(uintt32_t addr, u8 *p, u16 num)
{
    if(xW25Qxx .FlagInit ==0) return;    // 如果W25Qxx初始化失败，则跳过检测，防止卡死
...
{   
    ...
    {   
        uintt32_t Addr = 0x00;                  // 数据地址,  W25Q128最大地址:0X0100 0000
 ...
 *                  u16  num   连续读取的字节数 
**/
void W25qxx_ReadBuffer( uintt32_t addr, u8 *p, u16 num)
{
    if(xW25Qxx .FlagInit ==0) return;   // 如果W25Qxx初始化失败，则跳过检测，防止卡死
 ...
 * 参  数： u8   *p    要写入的数据存储区  
 *          uintt32_t  addr  写入地址         (W25Q128 只用3字节, W25Q256用4字节)
 *          u16  num   连续写入的字节数  
 * 返  回： 无
 * 备  注： 魔女开发板团队    资料更新Q群：1126717453    最后修改_2020年12月15日
 ******************************************************************************/  
u8 W25QXX_buffer[4096];                       // 开辟一段内存空间

void W25qxx_WriteBuffer( uintt32_t addr, u8* p, u16 num)
{
    ...
    uintt32_t  secPos      = addr/4096;             // 扇区地址,第几个扇区
...
void checkFlagGBKStorage(void)
{
    ...
    for(uintt32_t i=0; i< 6128640; i=i+1000000)                               
    {
