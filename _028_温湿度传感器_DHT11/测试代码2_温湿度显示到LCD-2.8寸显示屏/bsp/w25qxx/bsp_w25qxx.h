 ...
 **                  初始化  ：  vW25qx_Init()　
 **                  读取数据：  vW25qxx_ReadBuffer( uintt32_t addr, u8 *p, u16 num)　 // 数据地址，缓存指针，字节数
 **                  写入数据：  vW25qxx_WriteBuffer(uintt32_t addr, u8 *p, u16 num)   // 数据地址，缓存指针，字节数    
 **   
...
void W25qxx_ReadBuffer ( uintt32_t addr, u8 *p, u16 num);       // 读数据：addr-地址，*p-读取后数据缓存，num-要读取的字节数
void W25qxx_WriteBuffer( uintt32_t addr, u8 *p, u16 num);       // 写数据：addr-地址，*p-待写的数据缓存，num-要写入的字节数
