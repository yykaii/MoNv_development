...
void USART1_SendStringForDMA(char* stringTemp) 
{
    ...
    uintt32_t   num = 0;                               // 发送的数量，注意发送的单位不是必须8位的    
    char* t =stringTemp ;                        // 用于配合计算发送的数量    
    ...
    {   
        ...
        DMA1_Channel4->CMAR  = (uintt32_t)stringTemp;  // 存储器地址 
        DMA1_Channel4->CPAR  = (uintt32_t)&USART1->DR; // 外设地址      
    ...
    DMA1_Channel4->CCR  &= ~((uintt32_t)(1<<0));       // 失能，DMA必须失能才能配置
    DMA1_Channel4->CNDTR = num;                  // 传输数据量
    DMA1_Channel4->CMAR  = (uintt32_t)stringTemp;      // 存储器地址      
