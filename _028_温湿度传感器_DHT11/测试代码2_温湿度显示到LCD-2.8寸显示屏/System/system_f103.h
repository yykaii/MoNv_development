...
void  System_DelayMS (uintt32_t);                                                  // 毫秒延时  
void  System_DelayUS (uintt32_t);                                                  // 微秒延时
u64   System_GetTimeMs (void);                                               // 获取 SysTick 计时数, 单位:ms
uintt32_t   System_GetTimeInterval (void);                                         // 监察运行时间
void  System_TestRunTimes (void);                                            // printf打印监察运行时间 
// 简化初始化代码的3大函数
void  System_GPIOSet(GPIO_TypeDef* GPIOx, uintt32_t allPin, u8 mode, u8 speed);    // GPIO初始化
...
uintt32_t   System_GetSystemClock (void);                                          // 获取系统时钟频率
