...
{       
    ...
    uintt32_t msTick= SystemCoreClock /1000;     // 计算重载值，全局变量SystemCoreClock的值 ， 定义在system_stm32f10x.c    
 ...
 * 返回值： uintt32_t us
*****************************************************************************/
uintt32_t System_GetTimeUs(void)
{
    uintt32_t ms;
    uintt32_t us;
    do{
 ...
 * 参  数： uintt32_t ms : 需要延时的毫秒数
 * 返回值： 
*****************************************************************************/
void System_DelayMS(uintt32_t ms)
{    
 ...
 * 功  能： 微秒延时; 
 * 使  用： 在调用System_SysTickInit()函数后，即可使用 
 * 参  数： uintt32_t us : 需要延时的微秒数
 * 返回值：
*****************************************************************************/
void System_DelayUS(uintt32_t us)
{
 ...
 *****************************************************************************/ 
uintt32_t System_GetTimeInterval(void)
{
    static uintt32_t  lastTime=0, nowTime=0;
...
void System_TestRunTimes(void)
{
    ...
    uintt32_t intervalTimes = System_GetTimeInterval ();
    if(intervalTimes == 0)
    {
 ...
 * 返回值： 
******************************************************************************/
void System_GPIOSet(GPIO_TypeDef* GPIOx, uintt32_t allPin, u8 mode, u8 speed)
{
    uintt32_t reg = 0; 
    uintt32_t nowPin=0;    
...
//    }      
    ...
    // 配置寄存器CHL, 即PIN 0~7          
    for(uintt32_t i=0; i<8; i++) 
    {      
        nowPin = (uintt32_t) 0x01 << i;         // 当前要判断的引脚号     
    ...
    for(uintt32_t i=0; i<8; i++)    
    {      
        nowPin = (uintt32_t) 0x01 << (i+8);     // 当前要判断的引脚号     
        if((allPin & nowPin) != 0)        // 当前引脚要配置
        {
...
{    
    ...
    if(setGrouped ==0){
        ...
        SCB->AIRCR = ((uintt32_t)0x05FA0000)|(0x03<<8);   // 优先级分组设置, 已查,是3， F103和F429寄存器通用        
        setGrouped =1;
    }
...
{   
    SCB->AIRCR =0X05FA0000|(uintt32_t)0x04;      
}     
