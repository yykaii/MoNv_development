...
xDAC_TypeDef  xDAC;         // 声明为全局变量,方便记录信息、状态
...
{  
    ...
    xDAC.Channel1InitFlag =1;                                          // 标记通道1初始化标志
    DAC_PA4SetVoltage(0);                                              // 设置DAC通道1输出电压,mA  
}
...
{  
    ...
    xDAC.Channel2InitFlag =1;                                          // 标记通道2初始化标志       
    DAC_PA5SetVoltage(0);                                              // 设置DAC通道1输出电压,mA   
}



// 设置通道1电压值，单位：mA; 值域: 0~3300; 
void DAC_PA4SetVoltage(u16 mA)
{
    if(xDAC.Channel1InitFlag!=1)  
        DAC_Channel1Init();
...
void DAC_PA5SetVoltage(u16 mA)
{
    if(xDAC.Channel2InitFlag!=1)  
        DAC_Channel2Init();
    u16 dacValue=(float)mA/1000 /3.3 *4096;
